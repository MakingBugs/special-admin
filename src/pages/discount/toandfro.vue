    ...
    <el-card class="box-card">
        ...
        <div>
            <el-table :data="tableData" border fit highlight-current-row stripe style="width: 100%">
                ...
                <el-table-column prop="vehicleName" label="第一趟车辆类型" align="center"></el-table-column>
                <el-table-column prop="discountWayDesc" label="第一趟优惠方式" align="center"></el-table-column>
                <el-table-column prop="discountPrice" label="第一趟优惠值" align="center"></el-table-column>
                <el-table-column prop="secondVehicleName" label="第二趟车辆类型" align="center"></el-table-column>
                <el-table-column prop="secondDiscountWayDesc" label="第二趟优惠方式" align="center"></el-table-column>
                <el-table-column prop="secondDiscountPrice" label="第二趟优惠值" align="center"></el-table-column>
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="45%" @close="onDialogClose()">
            <el-form ref="dataForm" :rules="rules" :model="dataForm" label-width="150px">
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="服务区域" prop="zoneType">
                            <el-select v-model="dataForm.zoneType" placeholder="请选择" style="width: 80%;" :disabled="isdisabled" @change="DialogChangeRegion">
                                <el-option v-for="item in zoneTypeoptions" :key="item.value" :label="item.desc" :value="item.value"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第一趟车辆类型" prop="vehicleId">
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第一趟优惠方式" prop="discountWay">
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第一趟优惠值" prop="discountPrice">
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第二趟车辆类型" prop="secondVehicleId" :rules="[{required: true,message: '请选择第二趟车辆类型',trigger: 'change'}]">
                            <el-select v-model="secondVehicleId" clearable placeholder="请选择" style="width: 80%;" :disabled="isdisabled" @change="vehiclChange">
                                <el-option v-for="item in secondvehicleoptions" :value="item.value" :key="item.value" :label="item.desc">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第二趟优惠方式" prop="secondDiscountWay" :rules="[{required: true,message: '请选择第二趟优惠方式',trigger: 'radio'}]">
                            <el-radio v-model="secondDiscountWay" label="1">折扣（%）</el-radio>
                            <el-radio v-model="secondDiscountWay" label="2">折价（元）</el-radio>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="第二趟优惠值" prop="secondDiscountPrice" 
                        :rules="[{required: true,message: '请输入第二趟优惠值',trigger: 'blur'},
                        {pattern:/^[0-9]\d{0,2}?$/,message: '只能输入整数，最大值为999',trigger: 'blur'}]">
                            <el-input v-model="secondDiscountPrice" placeholder="请输入第二趟优惠值" style="width: 80%;" @blur="priceChange"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
export default {
    data(){
        return{
            ...
            vehicleoptions:[],//第一趟车辆类型
            secondvehicleoptions:[],//第二趟车辆类型
            stationoptions:[],//服务据点
            zoneTypeoptions:[],//服务区域
            secondVehicleId:null,//第二趟车辆类型
            secondDiscountWay:null,//第二趟优惠方式
            secondDiscountPrice:null,//第二趟优惠值   
            repetition:0,         
            //查询条件
            searchData: {
                zoneType: null,//区域
            ...
            dataForm:{
                ...
                startTime:'',//开始时间
                endTime:'',//结束时间
                vehicleId:null,//第一趟车辆类型
                discountWay:null,//第一趟优惠方式
                discountPrice:null,//第一趟优惠值
                secondVehicleId:null,//第二趟车辆类型
                secondDiscountWay:null,//第二趟优惠方式
                secondDiscountPrice:null,//第二趟优惠值
                //status:null,
    ...
    methods:{
        ...
        async initList(){
            ...
            await DiscountApi.discountList(this.searchData).then(res=>{
                res.list.map(i=>{
                    ...
                    if(i.secondDiscountWay==1){
                        i.secondDiscountPrice=i.secondDiscountPrice+'（%）'
                    }else if(i.secondDiscountWay==2){
                        i.secondDiscountPrice=i.secondDiscountPrice+'（元）'
                    }
                })
        ...
        async handleEdit(index, row){
            ...
            this.$nextTick(() => {
                ...
                //第二趟优惠方式
                if(row.secondDiscountWay==1){
                    this.secondDiscountWay='1';
                    this.dataForm.secondDiscountWay=this.secondDiscountWay;
                }else if(row.secondDiscountWay==2){
                    this.secondDiscountWay='2';
                    this.dataForm.secondDiscountWay=this.secondDiscountWay;
                }
                this.dataForm.discountId=row.discountId;//id
                ...
                this.dataForm.vehicleId=row.vehicleId;//第一趟车辆类型
                this.secondVehicleId=row.secondVehicleId;//第二趟车辆类型                
                this.dataForm.secondVehicleId=row.secondVehicleId;//第二趟车辆类型
                this.dataForm.discountPrice=row.discountPrice.split('（')[0];//第一趟优惠值
                this.secondDiscountPrice=row.secondDiscountPrice.split('（')[0];//第二趟优惠值
                this.dataForm.secondDiscountPrice=row.secondDiscountPrice.split('（')[0];//第二趟优惠值
                //this.dataForm.status=row.status;//状态
            })
        },
        //保存
        async onDialogSubmit() {
            this.dataForm.secondVehicleId=this.secondVehicleId;//第二趟车辆类型
            this.dataForm.secondDiscountWay= this.secondDiscountWay;//第二趟优惠方式
            this.dataForm.secondDiscountPrice=this.secondDiscountPrice;//第二趟优惠值
                    ...
                    if (new Date(this.dataForm.startTime).valueOf() > new Date(this.dataForm.endTime).valueOf()) {
                        ...
                        this.tableData.map(i=>{
                            if(this.dataForm.discountId==i.discountId){
                                ...
                                if((this.dataForm.zoneType==i.zoneType&&this.dataForm.stationId==i.stationId&&this.dataForm.vehicleId==i.vehicleId&&this.dataForm.secondVehicleId==i.secondVehicleId)&&
                        ...
                        if(this.repetition==1){
                            ...
                            //第一趟优惠方式
                            if(this.dataForm.discountWay=='1'){
                                this.dataForm.discountWay=1;
                            }else if(this.dataForm.discountWay=='2'){
                                this.dataForm.discountWay=2;
                            }
                            //第二趟优惠方式
                            if(this.dataForm.secondDiscountWay=='1'){
                                this.dataForm.secondDiscountWay=1;
                            }else if(this.dataForm.secondDiscountWay=='2'){
                                this.dataForm.secondDiscountWay=2;
                            }
        ...
        handleStatus(row){
            ...
            DiscountApi.discountStatus({discountId:row.discountId,status:status}).then(response=>{
                this.$message({
                message: tip,
                type: type,
                duration:1000,
        ...
        DialogChangeRegion(callback){
            if(callback==1){
                ...
                this.dataForm.stationId=null;
                this.searchData.stationId=null;
                this.dataForm.vehicleId=null;//第一趟车辆类型
                this.secondVehicleId=null;//第二趟车辆类型
            }else{
                this.stat=false;
                this.dataForm.stationId=0;
                this.searchData.stationId=null;
        ...
        async getcarType(id){
            this.vehicleoptions=[];
            this.secondvehicleoptions=[];
            this.dataForm.vehicleId=null;
            this.secondVehicleId=null;
            if(id==''||id==null){
                id=0;
            }
            await DiscountApi.discountGet(id).then(res=>{
                ...
                this.vehicleoptions=a;//第一趟车辆类型
                this.secondvehicleoptions=a;//第二趟车辆类型
            }).catch(err=>{console.log(err)})
        },
        //获取全部车辆类型
        async getCar(){
            ...
            await optionsList({dictionaryCodes:["VEHICLE_CLASS"],region:2}).then(res=>{
                this.vehicleoptions = res[0].list;//第一趟车辆类型————弹框
                this.secondvehicleoptions = res[0].list;//第二趟车辆类型————弹框
            }).catch(err=>{console.log(err)});
        },
        //跳转到指定页
        ...
        onDialogClose() {
            ...
            this.secondVehicleId=null;//第二次车辆类型
            this.secondDiscountWay=null;//第二次优惠方式
            this.secondDiscountPrice=null;//第二次优惠值
            this.vehicleoptions = [];//第一趟车辆类型————弹框
            this.secondvehicleoptions = [];//第二趟车辆类型————弹框
        ...
        //给form表单第二趟车辆类型赋值
        vehiclChange(id){
            this.dataForm.secondVehicleId=id;
        },
        //给form表单第二趟优惠值赋值
        priceChange(event){
            this.dataForm.secondDiscountPrice=event.target.value;
        }
    }
}
</script>